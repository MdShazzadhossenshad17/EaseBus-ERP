...
window.Orders = {
    ...
    async render(container) {
        container.innerHTML = `
            <!-- Print-Only Sales Orders Summary Header -->
            <div class="print-only mb-6 pb-4 border-b-2 border-slate-900">
                <div class="flex justify-between items-start">
                    <div class="flex items-center gap-3">
                        <img src="assets/img/logo.png" alt="EaseBus Logo" class="w-10 h-10 rounded-lg object-contain">
                        <div>
                            <h1 class="text-xl font-bold text-slate-900 tracking-tight">EaseBus ERP — Sales Orders & Operations Summary</h1>
                            <p class="text-xs text-slate-600">Official Sales Orders Ledger, Revenue KPI Metrics & Fulfillment Status</p>
                        </div>
                    </div>
                    <div class="text-right text-xs text-slate-600">
                        <div><strong>Document:</strong> Master Orders Summary</div>
                        <div><strong>Generated:</strong> <span id="print-orders-timestamp"></span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-2">
                    <button class="btn btn-secondary flex items-center gap-1 text-xs" onclick="Orders.printSummary()">
                        <span class="material-symbols-outlined text-sm">print</span> Print Orders Summary
                    </button>
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-300 animate-fade-in" id="invoice-printable">
                    ...
                    <div class="p-6 bg-slate-900 text-white flex justify-between items-center border-b border-slate-800">
                        <div class="flex items-center gap-3">
                            <img src="assets/img/logo.png" alt="EaseBus Logo" class="w-10 h-10 rounded-lg object-contain ring-1 ring-white/20">
                            <div>
                                <h2 class="font-geist font-extrabold text-xl tracking-tight text-white">${biz.name || 'EaseBus'}</h2>
                                <p class="text-xs text-slate-300 font-medium">Official Sales Tax Invoice & Order Summary</p>
                            </div>
                        </div>
                    ...
                    <div class="p-6 space-y-6 bg-white text-slate-900">
                        ...
                        <div class="print-only text-center text-[10px] text-slate-500 pt-4 border-t border-slate-300 mt-6">
                            <p class="font-semibold text-slate-800">Official Sales Tax Invoice & Order Fulfillment Summary</p>
                            <p class="mt-0.5">Generated by EaseBus ERP on behalf of ${biz.name || 'EaseBus'}. All rights reserved.</p>
                        </div>
                    </div>

                    <!-- Modal Actions Footer -->
                    <div class="p-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center print:hidden no-print">
    ...
    printSummary() {
        const timeEl = document.getElementById('print-orders-timestamp');
        if (timeEl) {
            timeEl.textContent = new Date().toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        }
        window.print();
    },
