...
window.Users = {
    ...
    rolesData: [
        ...
        { id: 2, name: 'sales', display_name: 'Sales Representative' },
        { id: 3, name: 'accountant', display_name: 'Staff Accountant' }
    ],
    searchTimeout: null,
    ...
    isOwner() {
        const u = API.getCurrentUser() || {};
        return typeof window.isStoreOwner === 'function' ? window.isStoreOwner(u) : (u.role === 'admin' || u.role === 'owner' || u.role === 'creator');
    },
        ...
        container.innerHTML = `
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 font-jakarta">
                <div>
                    ...
                    <p class="text-slate-400 text-xs mt-1 font-inter">${isOwner ? 'Real-time staff stream, attendance roster, and employee access permissions.' : 'Read-only staff directory & monthly shift roster. Modifying credentials requires Store Owner authorization.'}</p>
                </div>
            ...
            <div id="users-view-directory" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 font-jakarta" id="users-kpi-container">
                    ...
                    <div class="card p-5 bg-slate-900/90 border border-slate-800 border-l-4 border-l-indigo-500 shadow-xl rounded-2xl">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-[11px] font-bold uppercase tracking-wider text-indigo-400 font-outfit">Store Managers</p>
                                <h3 class="text-3xl font-extrabold text-indigo-400 mt-1.5 font-digit" id="kpi-active-admins">--</h3>
                                <p class="text-xs text-slate-400 mt-1 font-inter">Operations & stock management</p>
                            </div>
    ...
    filterUsers() {
        ...
        const getRoleBadge = (u) => {
            ...
            if (typeof window.isStoreOwner === 'function' && window.isStoreOwner(u)) {
        ...
        modal.innerHTML = `
            <div class="bg-slate-900 text-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-amber-500/40 animate-fade-in font-jakarta">
                <div class="p-6 text-center">
                    ...
                    <p class="text-xs text-slate-300 font-inter mb-6 leading-relaxed">As a store staff member (<span class="font-bold text-amber-300">${user.full_name || user.username || 'Staff'}</span>), you can inspect staff working activity logs in Read-Only mode. Creating new staff accounts or editing employee credentials requires authorization from the primary Store Owner.</p>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="role_name" class="form-input text-xs py-2" required>
                                    <option value="manager" ${user?.role_name === 'manager' || user?.role === 'manager' || user?.role === 'admin' ? 'selected' : ''}>Store Manager</option>
