...
window.Products = {
    ...
    async render(container) {
        ...
        const isCreator = window.isCreatorUser ? window.isCreatorUser(currentUser) : (currentUser.role === 'creator' || currentUser.username === 'shad@dbms.com');
        const isOwner = typeof window.isStoreOwner === 'function' ? window.isStoreOwner(currentUser) : (currentUser.role === 'admin' || currentUser.role === 'owner');

        const activeStoreName = (window.App && typeof window.App.getActiveStoreBusinessName === 'function')
            ? window.App.getActiveStoreBusinessName(currentUser)
            : (currentUser.business_name || 'My Store');
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 font-jakarta">
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ${isCreator ? `
                        <button class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-500/20 text-amber-300 border border-amber-400/30 cursor-pointer hover:bg-amber-500/30 transition-colors" onclick="App.showStoreSelectorModal()" title="Platform Creator: Click to switch monitored store">
                            <span class="material-symbols-outlined text-xs text-amber-400">shield</span>
                            <span>Monitored Store: <strong>${activeStoreName}</strong></span>
                            <span class="material-symbols-outlined text-xs text-amber-400">swap_horiz</span>
                        </button>
                        ` : (isOwner ? `
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-500/20 text-blue-300 border border-blue-400/30" title="Store Owner Workspace">
                            <span class="material-symbols-outlined text-xs text-blue-400">storefront</span>
                            <span>Store: <strong>${activeStoreName}</strong></span>
                        </span>
                        ` : `
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-slate-800 text-slate-300 border border-slate-700" title="Assigned Store">
                            <span class="w-2 h-2 rounded-full bg-emerald-400"></span>
                            <span>Store: <strong>${activeStoreName}</strong></span>
                            <span class="text-[10px] text-slate-400 font-normal">(Staff)</span>
                        </span>
                        `)}
                    </div>
                    <h1 class="text-2xl font-bold text-white tracking-tight">Products & Stock Upload</h1>
                    <p class="text-slate-400 text-xs mt-0.5 font-inter">Add new products and upload initial inventory stock to your store catalog.</p>
                </div>
                <div class="flex items-center gap-2 font-outfit">
                    ${isCreator ? `
                    <button class="btn btn-secondary flex items-center gap-1 text-xs py-2 px-3 bg-slate-800 text-slate-300 hover:text-white cursor-pointer" onclick="App.showStoreSelectorModal()" title="Platform Creator: Switch store tenant">
                        <span class="material-symbols-outlined text-sm text-amber-400">store</span> Switch Store
                    </button>
                    ` : ''}
                    <button class="btn btn-secondary flex items-center gap-1 text-xs py-2 px-3 bg-slate-800 text-slate-300 hover:text-white cursor-pointer" onclick="Products.loadSummary(); Products.loadProducts();">
        ...
        try {
            ...
            tbody.innerHTML = products.map(p => `
                <tr class="hover:bg-slate-50/80 text-xs">
                    <td class="font-mono-data font-bold text-slate-900 py-3">${p.sku || ('PRD-' + p.id)}</td>
                    <td class="py-3 font-semibold text-slate-900">${p.name || 'Unnamed Item'}</td>
                    <td class="py-3">
                    ...
                    <td class="data-number text-right font-bold text-slate-900 py-3">${UI.formatMoney(p.selling_price || 0)}</td>
                    <td class="data-number text-right font-mono py-3 font-bold ${(p.total_stock || 0) <= 5 ? 'text-amber-600' : 'text-slate-900'}">
                        ${p.total_stock !== undefined ? p.total_stock : (p.current_stock || 0)}
                    </td>
                    <td class="text-center py-3">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${
                            (p.status === 'active' || !p.status) ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-700 border-slate-200'
