...
window.Reports = {
            ...
            <div class="print-only mb-6 pb-4 border-b-2 border-slate-900">
                <div class="flex justify-between items-start">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-slate-900 tracking-tight">EaseBus ERP — Intelligence & Financial Performance Report</h1>
                            <p class="text-xs text-slate-600">Executive Profit & Loss Income Statement, Expense Allocation & Asset Valuation</p>
                        </div>
                    </div>
                    <div class="text-right text-xs text-slate-600">
                        <div><strong>Statement Period:</strong> <span id="print-statement-period">Last 7 Days</span></div>
                        <div><strong>Generated:</strong> <span id="print-report-timestamp"></span></div>
                    </div>
                </div>
            </div>
    ...
    setPreset(days, btnEl = null) {
        ...
        if (btnEl) {
            ...
            btnEl.style.setProperty('box-shadow', '0 2px 8px rgba(37, 99, 235, 0.4)', 'important');
        }
    ...
    async generatePL(showToast = false) {
        ...
        try {
            ...
            if (periodSpan && r.period) {
                const formatted = `${UI.formatDate(r.period.start || start)} - ${UI.formatDate(r.period.end || end)}`;
                periodSpan.textContent = formatted;
                const printPeriod = document.getElementById('print-statement-period');
                if (printPeriod) printPeriod.textContent = formatted;
            }
    ...
    printReport() {
        const periodSpan = document.getElementById('statement-period');
        const printPeriod = document.getElementById('print-statement-period');
        const printTime = document.getElementById('print-report-timestamp');
        if (printPeriod && periodSpan) {
            printPeriod.textContent = periodSpan.textContent || 'Current Period';
        }
        if (printTime) {
            printTime.textContent = new Date().toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        }
        window.print();
    }
};
